...
contract('ERC20', function (accounts) {
        ...
        it('a 1 valeur décimale', async function() {
        ...
        it("vérifie si un transfert a bien été effectué", async function() {
        ...
        it("vérifie si la fonction approve marche", async function() {
        ...
        it("Vérifie si transferFrom marche", async function(){

            //function transferFrom(address _from, address _to, uint256 _value) public returns (bool success)

            let amount1 = new BN(10);

            let ownerBalanceBeforeTransfer = await this.ERC20Instance.balanceOf(owner);
            let recipient2BalanceBeforeTransfer = await this.ERC20Instance.balanceOf(recipient2);

            await this.ERC20Instance.approve(recipient, amount1, {from:owner});
            await this.ERC20Instance.transferFrom(owner, recipient2, amount1, {from: recipient});

            let ownerBalanceAfterTransfer = await this.ERC20Instance.balanceOf(owner);
            let recipient2BalanceAfterTransfer = await this.ERC20Instance.balanceOf(recipient2);

            expect(ownerBalanceAfterTransfer).to.be.bignumber.equal(ownerBalanceBeforeTransfer.sub(amount1));
            expect(recipient2BalanceAfterTransfer).to.be.bignumber.equal(recipient2BalanceBeforeTransfer.add(amount1));
        });

        it("Checks the revert the transferFrom from", async function() {

            let amount = new BN(10);
            await expectRevert(this.ERC20Instance.transferFrom(owner, recipient, amount, {from: recipient}),
                "ERC20: transfer amount exceeds allowance");
        });

        it("vérifie la mise à jour de la fonction approve après le transferFrom", async function() {
